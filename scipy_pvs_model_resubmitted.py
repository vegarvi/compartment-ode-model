...
def f(p,t):
    if t <= rest:
        Q_tot = Q_prod
    elif t> rest and t < rest+infusion:
        Q_tot = Q_prod+Q_in
    else:
        Q_tot = Q_prod
    # MODIFICATION 8
    ...
    if p > p_cap:           # NO ABSORPTION BY CAPILLARIES
        p_pvs = p
    dp = C(p,E,pr)**-1*(Q_tot + (p_CP - p)/r_CP + (p_AG - p)/r_AG + (p_pvs - p)/r_a + (p_pvs - p)/r_lymph)
...
if table:
    ...
    print('%d %.2f %.2f %.2f & %.2f & %.2f \\\ '%(model, 1/mL_convert*(Q_prod+Q_in), max(p_CSF)/133.33, p_convert**-1*(max(p_CSF[idx0:idx1])-min(p_CSF[idx0:idx1]))/Q_in, Q[idx0,3], Q[idx1,3]))
else:
    print('Clinical R_out = ', p_convert**-1*(max(p_CSF[idx0:idx1])-min(p_CSF[idx0:idx1]))/Q_in)
