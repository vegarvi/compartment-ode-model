...
p_AG = 8.4*133.33
p_lymph = 8.4*133.33             # Wild guess
p_CP = 8.4*133.33                # Wild guess
p_cap = 20*133.33
...
for i in range(N-1):
    ...
    Q[i,:] = array([(p_CP - p[i,0])/r_CP, (p_AG - p[i,0])/r_AG, (p[i,1] - p[i,0])/r_a, (p_cap - p[i,1])/r_c, (p_lymph - p[i,1])/r_V])
    ...
    p[i+1,1] = p[i,1] + dt/C(p[i,1],E*10,p0)*( (p[i,0] - p[i,1])/r_a + (p_cap - p[i,1])/r_c + (p_lymph - p[i,1])/r_V)
